	...
	public boolean send(String ip, String msg, String command, int port) {
		try {
			...
			if (rt.findnext(hostIP, ip) == "error") {
				...
				InetAddress nexthop = InetAddress.getByName(rt.findnext(hostIP, ip));
				//发送消息到对方的对应端口
				...
				if (!command.equals("ACK")) {
					...
					Thread t = new RecvThread(ds, rt.findnext(hostIP, ip), command);
					t.start();
					threadPool.put(rt.findnext(hostIP, ip), t);
					t.join();
...
//				}
				...
				if (recvResponse) {
					...
					if (msgs[1].equals(hostIP)) {
						...
						send(rt.findnext(hostIP,msgs[1]), msgs[0] + "#" + msgs[1] + "#" + msgs[2] + "#" + msgs[3], "forward", ds.getLocalPort());
					}
				} else {
					//System.out.println("No response -- give up");
					rt.remove(hostIP,this.src);
...
//	} 
	...
	public void ServerAnalyse(String data, String srcIP, int port) {
		...
		if (msgs[3].equals("connect") && msgs.length == 5) {
			...
			for (String ip: ipPool) {
				String str = hostIP + "#" + ip + "#" + hostIP + "#" + srcIP;
				send(ip, str, "delete", port);
			}
			...
			send(srcIP, res, "ACK", port);
		} else if (msgs[3].equals("delete")) {
			rt.remove(msgs[0], msgs[3]);
			send(srcIP, res, "ACK", port);
		}
		else {
			send(srcIP, res, "ACK", port);
